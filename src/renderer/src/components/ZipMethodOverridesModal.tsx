...
import { ChevronRight, File, FileHeart, Files, Folder, Home, RotateCcw, X } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import type { ArchiveInputTreeEntry } from '@services/archiveInputTree'
import type { DeflateStrategy, ZipMethod, ZipMethodOverride } from '@services/compressor'
...
type MethodSelection = ZipMethod | 'mixed'
...
export const ZipMethodOverridesModal: React.FC<ZipMethodOverridesModalProps> = ({
  ...
  const methodLabel = (method: ZipMethod): string => {
  ...
  const selectionFor = (sourcePath: string, isDirectory: boolean): MethodSelection => {
    ...
    const methods = new Set<ZipMethod>([directRule?.method ?? effectiveMethod(sourcePath, true)])
    for (const rule of overrides) {
  ...
  const strategySelectionFor = (sourcePath: string, isDirectory: boolean): StrategySelection | null => {
    ...
    if (selectionFor(sourcePath, isDirectory) === 'mixed' || method !== 'deflate') return null
    const directRule = matchingRule(sourcePath, isDirectory ? 'tree' : 'file')
    ...
    for (const rule of overrides) {
      if (rule.method === 'deflate' && rule.deflateStrategy !== undefined &&
  ...
  const memorySelectionFor = (sourcePath: string, isDirectory: boolean): MemorySelection | null => {
    ...
    for (const rule of overrides) {
      if (rule.method === 'deflate' && rule.memLevel !== undefined &&
          isDescendant(sourcePath, rule.sourcePath, isWindows)) {
  ...
  const setMethod = (sourcePath: string, isDirectory: boolean, selection: MethodSelection) => {
    ...
    onChange([...base, {
      ...
      scope,
      method: selection,
      ...(selection === 'deflate' && directRule?.deflateStrategy !== undefined
        ? { deflateStrategy: directRule.deflateStrategy }
        : {}),
