...
describe('decoding LZMA2 as it arrives', () => {
  ...
  // The one-byte case is also what holds the buffer bookkeeping linear: it
  // leaves an entry per byte, so releasing them one at a time rather than in
  // one go cost the square of the queue and ran for most of a minute here.
