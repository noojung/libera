...
test('round trips an encrypted 7z, prompting for the password on extraction', async ({ app, page, workDir }) => {
  const source = path.join(workDir, 'source')
  await seedFiles(source, ['secret.bin'], 64 * 1024)
  const outputPath = path.join(workDir, 'out', 'locked.7z')
  ...
  const extractDir = path.join(workDir, 'unlocked')
  await page.locator('.extraction-panel__destination-row .input-text').fill(extractDir)
  await page.locator('.extraction-panel__start-button').click()

  // Headers are encrypted, so the app must ask before it can list anything.
  ...
  await expect(page.locator('.queue-manager__job--completed')).toHaveCount(2, { timeout: 60_000 })
  await expect(page.locator('.queue-manager__job--error')).toHaveCount(0)
  expect(await findFile(extractDir, 'secret.bin')).not.toBeNull()
})
